...
class Redsysoficial extends PaymentModule {
	...
	private function _postValidation() {
		...
		if (Tools::isSubmit ( 'btnSubmit' )) {
			...
			if (! Tools::getValue ( 'codigo' ) || ! checkFuc ( Tools::getValue ( 'codigo' ) ))
				$this->_postErrors [] = $this->l ( 'Se requiere el número de comercio (FUC) o el valor indicado para el número de comercio (FUC) no es correcto.' );
			if (! Tools::getValue ( 'clave256' ) || ! checkFirma ( Tools::getValue ( 'clave256' ) ))
				$this->_postErrors [] = $this->l ( 'Se requiere la Clave secreta de encriptación o el valor indicado para la Clave secreta de encriptación no es correcto.' );
			if (! Tools::getValue ( 'terminal' ) || ! checkTerminal ( Tools::getValue ( 'terminal' ) ))
				$this->_postErrors [] = $this->l ( 'Se requiere el Terminal del comercio o el valor indicado para el Terminal del comercio no es correcto.' );
			if (Tools::getValue('estado_pedido')==='-1')
				$this->_postErrors[] = $this->l('Ha de indicar un estado de pedido válido.');
		}
	}
	...
	{
		// lista de payments
		$this->_html .= '<img src="../modules/redsysoficial/img/redsys.png" style="float:left; margin-right:15px;"><b><br />'
					...
					<select name="estado_pedido" id="estado_pago_1">';
						...
						foreach ($order_states as $state) {
							if($state['unremovable'] == '1')
								$this->_html.='<option value="'.$state['id_order_state'].'" '.(($id_estado_pedido==$state['id_order_state'])?'selected':'').'>'.$state['name'].'</option>';
						}
	$this->_html.='	</select>
			</td>
		</tr>
