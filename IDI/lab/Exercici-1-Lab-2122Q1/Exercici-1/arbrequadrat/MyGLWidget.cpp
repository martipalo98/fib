...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    /*case Qt::Key_C :
        scl += 0.5;
        GLint varLoc = glGetUniformLocation (program->programId(),"val");
...
void MyGLWidget::modelTransformArbre (const glm::vec3& posicio, float mida)
{
  ...
  //TG = glm::translate (TG, posicioArbre[arbre]);
  TG = glm::rotate (TG, float(-M_PI/6*hores), glm::vec3(0.0, 0.0, 1.0));
  scl+=0.1;
...
void MyGLWidget::carregaShaders()
{
  ...
  GLint varLoc = glGetUniformLocation (program->programId(),"val");
  glUniform1f (varLoc, 0);


  //colorTronc
  GLint varTronc = glGetUniformLocation (program->programId(),"colorTronc");
  glm::vec3 colTronc = glm::vec3(0.6, 0.38, 0.25);
  glUniform3fv (varTronc, 1, &colTronc[0]);
}
